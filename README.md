...
Three independent skill files, each a complete markdown-based framework an AI agent reads and follows. Use one, two, or all three depending on your investing style.
...
All three skills call the same five scripts for live data instead of relying on an LLM's training-data memory of stock prices (which is always stale) or unstructured web search (which is slow and inconsistent to parse):
...
- **`reddit_sentiment.py`** -- subreddit search for ticker discussion, used strictly as a discovery/confirmation signal, never as a decision input. Powered by PRAW, free for personal/research use (100 requests/min). Requires a free Reddit app registration.
...
2. Point your agent's skill loader at one of the three `SKILL.md` files, or all three.
3. Set up credentials only for the scripts you need:
   ...
   - `sec_filings.py` needs an identity string (your name and email, per SEC's fair-access policy -- not a real account).
   - `reddit_sentiment.py` needs a free Reddit app (reddit.com/prefs/apps).
...
Credentials can be passed in the JSON payload or set as environment variables (`REDDIT_CLIENT_ID`/`REDDIT_CLIENT_SECRET`, `EDGAR_IDENTITY`, `PERPLEXITY_API_KEY`) -- never hardcoded in the scripts.
...
- Each skill is fully self-contained -- you can install just one if you only care about swing trading, or just the long-term compounder framework, without needing the others.
...
MIT. See individual script headers for third-party library attributions (`yfinance`, `edgartools`, `PRAW`, Perplexity Sonar API).
